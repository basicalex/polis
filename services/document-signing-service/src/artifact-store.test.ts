...
test('S3 artifact retrieval rejects bytes that do not match the stored hash', async () => {
  ...
  const ref = await store.put(source, { artifactId: 'artifact-1', sha256: sha256Hex(source), mimeType: 'application/pdf' });
  assert.match(ref, /^s3:/);
...
test('artifact store selection fails fast for incomplete S3 configuration', () => {
  assert.throws(
    () => createArtifactStore({} as DbClient, { ARTIFACT_STORE_MODE: 's3', S3_ARTIFACT_ENDPOINT: 'http://minio.test' }),
    /configuration is incomplete/,
  );
  assert.throws(() => createArtifactStore({} as DbClient, { ARTIFACT_STORE_MODE: 'public-url' }), /Unsupported ARTIFACT_STORE_MODE/);
});
