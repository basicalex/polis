...
export interface SigningRepository {
  ...
  insertArtifact(
    artifact: Omit<DocumentArtifactRecord, 'createdAt'>,
  ): Promise<DocumentArtifactRecord>;
  getArtifact(id: string): Promise<DocumentArtifactRecord | null>;
  findArtifact(
    subjectId: string,
    kind: DocumentArtifactRecord['kind'],
  ): Promise<DocumentArtifactRecord | null>;
  findArtifactByNaturalKey(
    input: Pick<DocumentArtifactRecord, 'subjectType' | 'subjectId' | 'kind' | 'version'>,
  ): Promise<DocumentArtifactRecord | null>;
  createSigningRequest(input: CreateSigningRequestInput): Promise<SigningRequestRecord>;
  findSigningRequestByIdempotencyKey(
    mandateHolderId: string,
    key: string,
  ...
  findSigningRequestByProviderEnvelopeId(
    provider: 'stub' | 'documenso',
    envelopeId: string,
  ): Promise<SigningRequestRecord | null>;
  updateSigningRequest(
    id: string,
    patch: SigningRequestPatch,
  ): Promise<SigningRequestRecord | null>;
  insertRecipient(
    input: Omit<SigningRecipientRecord, 'id'> & { id?: string },
  ): Promise<SigningRecipientRecord>;
  ...
  insertCharterEvent(input: {
    ...
    signingRequestId?: string | null;
    event:
      'initiated' | 'distributed' | 'completed' | 'accepted' | 'declined' | 'expired' | 'withdrawn';
    actorId?: string | null;
  }): Promise<void>;
...
function requestRow(
  row: typeof requestSelect extends Record<string, infer _> ? Record<string, unknown> : never,
...
export class DrizzleSigningRepository implements SigningRepository {
  ...
  async loadSigningContext(mandateHolderId: string): Promise<SigningContext | null> {
      ...
      this.db
        ...
        .where(
          and(
            ...
            eq(schema.mandateHolderCharters.status, 'pending'),
          ),
        )
        .orderBy(desc(schema.mandateHolderCharters.version))
        .limit(1),
      this.db
        .select()
        .from(schema.citizens)
        .where(eq(schema.citizens.id, holder.citizenId))
        .limit(1),
    ]);
    ...
    return {
      ...
      charter: {
        id: charter.id,
        mandateHolderId: charter.mandateHolderId,
        charterDoc: charter.charterDoc,
      ...
      citizen: {
        ...
        identityLevel: citizen.identityLevel,
      },
    };
  }

  async insertArtifact(
    input: Omit<DocumentArtifactRecord, 'createdAt'>,
  ): Promise<DocumentArtifactRecord> {
  ...
  async getArtifact(id: string): Promise<DocumentArtifactRecord | null> {
    const rows = await this.db
      ...
      .where(eq(schema.documentArtifacts.id, id))
      .limit(1);
  ...
  async findArtifact(
    ...
    const rows = await this.db
      ...
      .where(
        and(
          eq(schema.documentArtifacts.subjectId, subjectId),
          eq(schema.documentArtifacts.kind, kind),
        ),
      )
      .orderBy(desc(schema.documentArtifacts.version))
      .limit(1);
    return (rows[0] as DocumentArtifactRecord | undefined) ?? null;
  }

  async findArtifactByNaturalKey(
    input: Pick<DocumentArtifactRecord, 'subjectType' | 'subjectId' | 'kind' | 'version'>,
  ): Promise<DocumentArtifactRecord | null> {
    const rows = await this.db
      .select()
  ...
  async createSigningRequest(input: CreateSigningRequestInput): Promise<SigningRequestRecord> {
    const rows = await this.db
      .insert(schema.signingRequests)
      .values({ ...input, id: input.id ?? randomUUID() })
      .onConflictDoNothing()
      .returning(requestSelect);
    if (rows[0]) return requestRow(rows[0]);
    const existing = await this.findSigningRequestByIdempotencyKey(
      input.mandateHolderId,
      input.idempotencyKey,
    );
    if (existing) return existing;
    if (await this.hasIdempotencyKey(input.idempotencyKey))
      throw new SigningRequestIdempotencyConflictError();
    throw new Error('signing_request_insert_conflict');
  }

  async findSigningRequestByIdempotencyKey(
    mandateHolderId: string,
    key: string,
  ): Promise<SigningRequestRecord | null> {
    const rows = await this.db
  ...
  async hasIdempotencyKey(key: string): Promise<boolean> {
    const rows = await this.db
      .select({ id: schema.signingRequests.id })
      .from(schema.signingRequests)
      .where(eq(schema.signingRequests.idempotencyKey, key))
      .limit(1);
  ...
  async getSigningRequest(id: string): Promise<SigningRequestRecord | null> {
    const rows = await this.db
      .select(requestSelect)
      .from(schema.signingRequests)
      .where(eq(schema.signingRequests.id, id))
      .limit(1);
    return rows[0] ? requestRow(rows[0]) : null;
  }

  async getLatestSigningRequestForHolder(
    mandateHolderId: string,
  ): Promise<SigningRequestRecord | null> {
    const rows = await this.db
      .select(requestSelect)
      .from(schema.signingRequests)
      .where(eq(schema.signingRequests.mandateHolderId, mandateHolderId))
      .orderBy(desc(schema.signingRequests.createdAt))
      .limit(1);
    return rows[0] ? requestRow(rows[0]) : null;
  }
  ...
  async findSigningRequestByProviderEnvelopeId(
    provider: 'stub' | 'documenso',
    envelopeId: string,
  ): Promise<SigningRequestRecord | null> {
    const rows = await this.db
  ...
  async updateSigningRequest(
    id: string,
    patch: SigningRequestPatch,
  ): Promise<SigningRequestRecord | null> {
    const values: Record<string, unknown> = { updatedAt: new Date() };
    if (patch.providerEnvelopeId !== undefined)
      values.providerEnvelopeId = patch.providerEnvelopeId;
    ...
    if (patch.providerCompletedAt !== undefined)
      values.providerCompletedAt = patch.providerCompletedAt;
    ...
    const rows = await this.db
      .update(schema.signingRequests)
      .set(values)
      .where(
        and(
          eq(schema.signingRequests.id, id),
          notInArray(schema.signingRequests.status, [...TERMINAL_SIGNING_STATUSES]),
        ),
      )
      .returning(requestSelect);
  ...
  async insertRecipient(
    ...
    const rows = await this.db
      .insert(schema.signingRecipients)
      .values({ ...input, id: input.id ?? randomUUID() })
      .onConflictDoNothing()
      .returning();
    ...
    const match = existing.find(
      (row) => row.citizenId === input.citizenId && row.role === input.role,
    );
  ...
  async listRecipients(requestId: string): Promise<SigningRecipientRecord[]> {
    return (await this.db
      .select()
      .from(schema.signingRecipients)
      .where(eq(schema.signingRecipients.signingRequestId, requestId))
      .orderBy(schema.signingRecipients.signingOrder)) as SigningRecipientRecord[];
  }

  async updateRecipientProviderId(id: string, providerRecipientId: string): Promise<void> {
    await this.db
      .update(schema.signingRecipients)
      .set({ providerRecipientId, status: 'sent', updatedAt: new Date() })
      .where(eq(schema.signingRecipients.id, id));
  }
  ...
  async recordProviderEvent(input: {
    ...
    const rows = await this.db
      .insert(schema.signingProviderEvents)
      .values({
        id: randomUUID(),
        provider: input.provider,
        signingRequestId: input.signingRequestId ?? null,
      ...
      .onConflictDoNothing()
      .returning({ id: schema.signingProviderEvents.id });
    return rows.length === 1;
  }
  ...
  async countUnresolvedProviderEvents(provider: 'stub' | 'documenso'): Promise<number> {
    const rows = await this.db
      .select({ value: count() })
      .from(schema.signingProviderEvents)
      .where(
        and(
  ...
  async listDueRequests(limit: number, maxAttempts: number): Promise<SigningRequestRecord[]> {
    const rows = await this.db
      ...
      .orderBy(schema.signingRequests.lastReconciledAt, schema.signingRequests.createdAt)
      .limit(limit);
    return rows.map(requestRow);
  }

  async insertCharterEvent(input: {
    charterId: string;
    signingRequestId?: string | null;
    event:
      'initiated' | 'distributed' | 'completed' | 'accepted' | 'declined' | 'expired' | 'withdrawn';
    ...
    await this.db.insert(schema.mandateHolderCharterEvents).values({
      id: randomUUID(),
  ...
  async finalizeSigning(input: FinalizeSigningInput): Promise<SigningRequestRecord> {
    return this.db.transaction(async (tx) => {
      const currentRows = await tx
        .select(requestSelect)
        .from(schema.signingRequests)
        .where(eq(schema.signingRequests.id, input.requestId))
        .limit(1);
      ...
      if (TERMINAL_SIGNING_STATUSES.includes(current.status))
        throw new Error('signing_request_terminal');
      const completedRows = await tx
        .update(schema.signingRequests)
        .set({
          status: 'completed',
          signedArtifactId: input.signedArtifactId,
          proofManifestId: input.proofManifestId,
          providerCompletedAt: input.completedAt,
          lastReconciledAt: input.completedAt,
          reconcileAttempts: sql`${schema.signingRequests.reconcileAttempts} + 1`,
          updatedAt: input.completedAt,
          failureCode: null,
        })
        .where(
          and(
            eq(schema.signingRequests.id, input.requestId),
            notInArray(schema.signingRequests.status, [...TERMINAL_SIGNING_STATUSES]),
          ),
        )
        .returning(requestSelect);
      ...
      const acceptedRows = await tx
        .update(schema.mandateHolderCharters)
        .set({
          status: 'accepted',
          acceptedSigningRequestId: input.requestId,
          signedArtifactId: input.signedArtifactId,
          proofManifestId: input.proofManifestId,
        ...
        .where(
          and(
            eq(schema.mandateHolderCharters.id, current.charterId),
            eq(schema.mandateHolderCharters.status, 'pending'),
          ),
        )
        .returning({ id: schema.mandateHolderCharters.id });
...
export class InMemorySigningRepository implements SigningRepository {
  ...
  private readonly unresolvedProviderEventCount: Record<'stub' | 'documenso', number> = {
    stub: 0,
    documenso: 0,
  };
  readonly charterEvents: Array<{
    charterId: string;
    signingRequestId: string | null;
    event: string;
    actorId: string | null;
  }> = [];
  readonly acceptedCharters = new Map<
    string,
    { requestId: string; signedArtifactId: string; proofManifestId: string; signedAt: Date }
  >();

  constructor(seed: InMemorySigningSeed = {}) {
    for (const context of seed.contexts ?? [])
      this.contexts.set(context.holder.id, structuredClone(context));
    ...
    for (const recipient of seed.recipients ?? [])
      this.recipients.set(recipient.id, structuredClone(recipient));
  }

  async loadSigningContext(id: string): Promise<SigningContext | null> {
  ...
  async getArtifact(id: string): Promise<DocumentArtifactRecord | null> {
    return structuredClone(this.artifacts.get(id) ?? null);
  }
  async findArtifact(
    subjectId: string,
    kind: DocumentArtifactRecord['kind'],
    ...
    const rows = [...this.artifacts.values()]
      .filter((row) => row.subjectId === subjectId && row.kind === kind)
  ...
  async findSigningRequestByIdempotencyKey(
    ...
    return structuredClone(
      [...this.requests.values()].find(
        (row) => row.mandateHolderId === mandateHolderId && row.idempotencyKey === key,
      ) ?? null,
    );
  }
  async hasIdempotencyKey(key: string): Promise<boolean> {
    return [...this.requests.values()].some((row) => row.idempotencyKey === key);
  }
  async getSigningRequest(id: string): Promise<SigningRequestRecord | null> {
    return structuredClone(this.requests.get(id) ?? null);
  }
  async getLatestSigningRequestForHolder(id: string): Promise<SigningRequestRecord | null> {
    return structuredClone(
      [...this.requests.values()]
        .filter((row) => row.mandateHolderId === id)
        .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0] ?? null,
    );
  }
  async getMandateHolderCitizenId(id: string): Promise<string | null> {
    return this.contexts.get(id)?.holder.citizenId ?? null;
  }
  async findSigningRequestByProviderEnvelopeId(
    provider: 'stub' | 'documenso',
    envelopeId: string,
    ...
    return structuredClone(
      [...this.requests.values()].find(
        (row) => row.provider === provider && row.providerEnvelopeId === envelopeId,
      ) ?? null,
    );
  }
  async updateSigningRequest(
    ...
    if (patch.providerCompletedAt !== undefined)
      row.providerCompletedAt = patch.providerCompletedAt;
  ...
  async insertRecipient(
    ...
    const existing = [...this.recipients.values()].find(
      (row) =>
        row.signingRequestId === input.signingRequestId &&
  ...
  async listRecipients(requestId: string): Promise<SigningRecipientRecord[]> {
    return structuredClone(
      [...this.recipients.values()]
        .filter((row) => row.signingRequestId === requestId)
  ...
  async countUnresolvedProviderEvents(provider: 'stub' | 'documenso'): Promise<number> {
    return this.unresolvedProviderEventCount[provider];
  }
  async listDueRequests(limit: number, maxAttempts: number): Promise<SigningRequestRecord[]> {
    return structuredClone(
      [...this.requests.values()]
        .filter(
          (row) =>
            ['created', 'distributed', 'awaiting_signatures'].includes(row.status) &&
  ...
  async finalizeSigning(input: FinalizeSigningInput): Promise<SigningRequestRecord> {
    ...
    row.signedArtifactId = input.signedArtifactId;
    row.proofManifestId = input.proofManifestId;
    row.providerCompletedAt = input.completedAt;
    row.lastReconciledAt = input.completedAt;
    row.reconcileAttempts += 1;
    ...
    if (context?.charter.status === 'pending') {
      ...
      this.acceptedCharters.set(row.charterId, {
        requestId: row.id,
        signedArtifactId: input.signedArtifactId,
        proofManifestId: input.proofManifestId,
        signedAt: input.completedAt,
