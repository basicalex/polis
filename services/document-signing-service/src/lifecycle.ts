...
function charterDocument(value: unknown): Readonly<Record<string, unknown>> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new SigningLifecycleError('charter_document_missing', 409);
...
function providerEnvelopeId(rawBody: Uint8Array): string | null {
  try {
    ...
    const data = root.data && typeof root.data === 'object' && !Array.isArray(root.data)
      ...
      : null;
    const rootEnvelope = root.envelope && typeof root.envelope === 'object' && !Array.isArray(root.envelope)
      ? root.envelope as Record<string, unknown>
      : null;
    const dataEnvelope = data?.envelope && typeof data.envelope === 'object' && !Array.isArray(data.envelope)
...
export class SigningLifecycle {
  ...
  async initiateCharterSigning(input: InitiateCharterSigningInput): Promise<SigningRequestRecord> {
    if (!input.idempotencyKey.trim()) throw new SigningLifecycleError('idempotency_key_required', 400);
    ...
    if (context.holder.citizenId !== input.actorCitizenId) throw new SigningLifecycleError('mandate_holder_owner_required', 403);
    if (context.holder.status !== 'active') throw new SigningLifecycleError('mandate_holder_not_active', 409);
    if (context.citizen.identityLevel !== 'verified_official') throw new SigningLifecycleError('verified_official_required', 403);

    const existing = await this.#repository.findSigningRequestByIdempotencyKey(input.mandateHolderId, input.idempotencyKey);
    ...
    const rendered = renderCharterPdf(charterDocument(context.charter.charterDoc), [{ displayName: context.holder.displayName, mandate: context.holder.roleId ?? undefined }]);
    ...
    if (unsignedArtifact) {
      ...
      if (unsignedArtifact.sha256 !== unsignedHash || sha256Hex(existingBytes) !== unsignedArtifact.sha256) {
        throw new SigningLifecycleError('artifact_hash_mismatch', 409);
      }
    } else {
      const unsignedId = randomUUID();
      const unsignedStorageRef = await this.#store.put(rendered.pdf, { artifactId: unsignedId, sha256: unsignedHash, mimeType: 'application/pdf' });
      ...
      if (unsignedArtifact.sha256 !== unsignedHash) throw new SigningLifecycleError('artifact_hash_mismatch', 409);
    }

    let request: SigningRequestRecord;
    try {
      request = await this.#repository.createSigningRequest({ provider: this.#providerName, charterId: context.charter.id, mandateHolderId: context.holder.id, unsignedArtifactId: unsignedArtifact.id, idempotencyKey: input.idempotencyKey });
    } catch (error) {
    ...
    const recipient = await this.#repository.insertRecipient({ signingRequestId: request.id, citizenId: context.citizen.id, role: 'signer', signingOrder: 1, providerRecipientId: null, status: 'pending' });
    await this.#repository.insertCharterEvent({ charterId: context.charter.id, signingRequestId: request.id, event: 'initiated', actorId: input.actorCitizenId });
    ...
    if (envelope.recipients[0]) await this.#repository.updateRecipientProviderId(recipient.id, envelope.recipients[0].id);
    const distributed = await this.#provider.distributeEnvelope(envelope.id);
    const updated = await this.#repository.updateSigningRequest(request.id, { status: distributed.state === 'pending' ? 'distributed' : STATE_TO_REQUEST_STATUS[distributed.state] });
    await this.#repository.insertCharterEvent({ charterId: context.charter.id, signingRequestId: request.id, event: 'distributed', actorId: input.actorCitizenId });
    await this.#emitAudit({ lifecycle: 'charter_signing_distributed', requestId: request.id, charterId: context.charter.id, mandateHolderId: context.holder.id, actorCitizenId: input.actorCitizenId });
    return updated ?? request;
  }

  async recordProviderEvent(rawBody: Uint8Array, eventName: string, maxUnresolvedReceipts = 100): Promise<boolean> {
    ...
    try {
      ...
      const request = envelopeId
        ? await this.#repository.findSigningRequestByProviderEnvelopeId(this.#providerName, envelopeId)
        : null;
      if (!request && await this.#repository.countUnresolvedProviderEvents(this.#providerName) >= maxUnresolvedReceipts) {
        return false;
      }
      const payloadHash = sha256Hex(rawBody);
      const eventFingerprint = createHash('sha256').update(this.#providerName).update('\0').update(eventName).update('\0').update(rawBody).digest('hex');
      return await this.#repository.recordProviderEvent({ provider: this.#providerName, signingRequestId: request?.id, eventName, eventFingerprint, payloadHash });
    } finally {
      release();
    }
  }
  ...
  async #reconcileUnlocked(requestId: string): Promise<SigningRequestRecord> {
    ...
    if (envelope.state === 'rejected' || envelope.state === 'expired' || envelope.state === 'cancelled') {
      const status = STATE_TO_REQUEST_STATUS[envelope.state];
      const updated = await this.#repository.updateSigningRequest(request.id, { status, reconciled: true, failureCode: null });
      const event = envelope.state === 'rejected' ? 'declined' : envelope.state === 'expired' ? 'expired' : 'withdrawn';
      await this.#repository.insertCharterEvent({ charterId: request.charterId, signingRequestId: request.id, event });
      await this.#emitAudit({ lifecycle: `charter_signing_${event}`, requestId: request.id, charterId: request.charterId, mandateHolderId: request.mandateHolderId });
      return updated ?? request;
    }

    const observed = STATE_TO_REQUEST_STATUS[envelope.state];
    if (request.status === 'created' || request.status === 'distributed' || request.status === 'awaiting_signatures') {
      request = (await this.#repository.updateSigningRequest(request.id, { status: observed, reconciled: true, failureCode: null })) ?? request;
    }
    return request;
  }

  async #complete(request: SigningRequestRecord, envelope: SigningEnvelope): Promise<SigningRequestRecord> {
    let signedArtifact = request.signedArtifactId ? await this.#repository.getArtifact(request.signedArtifactId) : null;
    ...
    if (!signedArtifact) {
      ...
      try {
        const storageRef = await this.#store.put(bytes, { artifactId, sha256: signedHash, mimeType: 'application/pdf' });
        ...
        await this.#repository.updateSigningRequest(request.id, { signedArtifactId: signedArtifact.id, providerCompletedAt: this.#now() });
      } catch (error) {
        await this.#repository.updateSigningRequest(request.id, { failureCode: 'artifact_store_error', reconciled: true });
        throw error;
      }
    }
    ...
    if (this.#paperless) {
      try { await this.#paperless.archive({ bytes: exactBytes, filename: signedArtifact.filename ?? 'signed-charter.pdf', documentClass: 'restricted-administrative-record' }); } catch { /* best-effort */ }
    }

    request = await this.#repository.getSigningRequest(request.id) ?? request;
    let proofManifestId = request.proofManifestId;
    if (!proofManifestId) {
      const manifestHash = createHash('sha256').update(`charter-proof-v1\0${request.id}\0${signedArtifact.sha256}`).digest('hex');
    ...
    const completed = await this.#repository.finalizeSigning({ requestId: request.id, signedArtifactId: signedArtifact.id, proofManifestId, completedAt: this.#now() });
    await this.#emitAudit({ lifecycle: 'charter_signing_completed', requestId: request.id, charterId: request.charterId, mandateHolderId: request.mandateHolderId });
    return completed;
  }

  async #emitAudit(input: { lifecycle: string; requestId?: string; charterId?: string; mandateHolderId?: string; actorCitizenId?: string }): Promise<void> {
    if (!this.#audit) return;
    try { await this.#audit.emit(input); } catch { /* best-effort */ }
  }
}
