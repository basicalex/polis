...
import { SigningLifecycle, SigningLifecycleError, type ProofRegistrationInput } from './lifecycle.js';
import { InMemorySigningRepository, type SigningContext, type SigningRequestRecord } from './repository.js';
...
class MemoryArtifactStore implements ArtifactStore {
  ...
  async put(bytes: Uint8Array, metadata: ArtifactPutMetadata): Promise<string> {
    if (this.failNextPut) { this.failNextPut = false; throw new Error('storage_unavailable'); }
  ...
  async exists(ref: string): Promise<boolean> { return this.bytes.has(ref); }
}

const context: SigningContext = {
  holder: { id: 'holder-1', citizenId: 'citizen-1', displayName: 'Official One', status: 'active', roleId: 'mayor' },
  charter: { id: 'charter-1', mandateHolderId: 'holder-1', charterDoc: { purpose: 'public reporting', scope: { jurisdictions: ['jurisdiction-1'], processes: ['process-1'] } }, version: 1, status: 'pending' },
  citizen: { id: 'citizen-1', email: 'official@example.test', displayName: 'Official One', identityLevel: 'verified_official' },
};
...
function fixture(contexts: SigningContext[] = [context]) {
  ...
  const lifecycle = new SigningLifecycle({
    ...
    providerName: 'stub',
    artifactStore: store,
    proofRegistrar: { register: async (input) => { proofInputs.push(input); return { id: `proof-${proofInputs.length}` }; } },
    paperlessArchiver: { archive: async ({ bytes }) => { paperlessBytes.push(Uint8Array.from(bytes)); } },
  });
...
test('stub charter flow finalizes once and preserves exact signed bytes', async () => {
  const f = fixture();
  const initiated = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'key-1' });
  assert.equal(initiated.status, 'distributed');
  const duplicate = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'key-1' });
  ...
  const signed = [...f.repository.artifacts.values()].filter((artifact) => artifact.kind === 'charter_signed');
  ...
  assert.equal([...f.repository.artifacts.values()].filter((artifact) => artifact.kind === 'charter_signed').length, 1);
  assert.equal(f.proofInputs.length, 1);
  assert.equal(f.repository.acceptedCharters.size, 1);
});
...
test('webhook receipts deduplicate and never override authoritative pending state', async () => {
  const f = fixture();
  const request = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'key-webhook' });
...
test('stale observations cannot regress a completed request', async () => {
  const f = fixture();
  const request = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'key-stale' });
  f.provider.completeForTest(request.providerEnvelopeId!);
  await f.lifecycle.reconcile(request.id);
  const stale = await f.repository.updateSigningRequest(request.id, { status: 'awaiting_signatures', reconciled: true });
...
test('storage failure leaves completion retryable', async () => {
  const f = fixture();
  const request = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'key-retry' });
  f.provider.completeForTest(request.providerEnvelopeId!);
...
test('non-owner cannot initiate charter signing', async () => {
  ...
  await assert.rejects(
    f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-2', idempotencyKey: 'key-owner' }),
    (error: unknown) => error instanceof SigningLifecycleError && error.code === 'mandate_holder_owner_required',
  );
...
test('same idempotency key cannot cross mandate-holder boundaries', async () => {
  const second: SigningContext = {
    holder: { id: 'holder-2', citizenId: 'citizen-2', displayName: 'Official Two', status: 'active', roleId: 'councillor' },
    charter: { id: 'charter-2', mandateHolderId: 'holder-2', charterDoc: { purpose: 'publish minutes', scope: { jurisdictions: ['jurisdiction-2'] } }, version: 1, status: 'pending' },
    citizen: { id: 'citizen-2', email: 'second@example.test', displayName: 'Official Two', identityLevel: 'verified_official' },
  };
  const f = fixture([context, second]);
  const first = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'shared-key' });
  await assert.rejects(
    f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-2', actorCitizenId: 'citizen-2', idempotencyKey: 'shared-key' }),
    (error: unknown) => error instanceof SigningLifecycleError && error.code === 'idempotency_key_conflict' && error.status === 409,
  );
  assert.equal(f.repository.requests.size, 1);
...
test('retry after a terminal outcome reuses the unsigned artifact', async () => {
  const f = fixture();
  const first = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'terminal-key-1' });
  await f.repository.updateSigningRequest(first.id, { status: 'expired', reconciled: true });

  const retried = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'terminal-key-2' });
  assert.notEqual(retried.id, first.id);
  assert.equal(retried.status, 'distributed');
  assert.equal(f.repository.requests.size, 2);
  assert.equal([...f.repository.artifacts.values()].filter((artifact) => artifact.kind === 'charter_unsigned').length, 1);
});

test('concurrent reconciliation registers one proof and one terminal event pair', async () => {
  const f = fixture();
  const request = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'concurrent-key' });
  f.provider.completeForTest(request.providerEnvelopeId!);
  const [left, right] = await Promise.all([f.lifecycle.reconcile(request.id), f.lifecycle.reconcile(request.id)]);
...
test('webhook receipts associate known envelopes and cap unresolved events', async () => {
  const f = fixture();
  const request = await f.lifecycle.initiateCharterSigning({ mandateHolderId: 'holder-1', actorCitizenId: 'citizen-1', idempotencyKey: 'receipt-key' });
  const known = Buffer.from(JSON.stringify({ data: { envelope: { id: request.providerEnvelopeId } } }));
  ...
  assert.equal(await f.lifecycle.recordProviderEvent(Buffer.from('{"envelopeId":"unknown-1"}'), 'completed', 1), true);
  assert.equal(await f.lifecycle.recordProviderEvent(Buffer.from('{"envelopeId":"unknown-2"}'), 'completed', 1), false);
...
test('concurrent unresolved webhook receipts cannot exceed the cap', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 20 }, (_, index) =>
      f.lifecycle.recordProviderEvent(Buffer.from(`{"envelopeId":"unknown-${index}"}`), 'completed', 1),
    ),
  );
...
test('accepted charter guard emits one acceptance across competing requests', async () => {
  ...
  const repository = new InMemorySigningRepository({ contexts: [context], requests: [first, second] });
  await repository.finalizeSigning({ requestId: first.id, signedArtifactId: 'signed-a', proofManifestId: 'proof-a', completedAt: new Date() });
  await repository.finalizeSigning({ requestId: second.id, signedArtifactId: 'signed-b', proofManifestId: 'proof-b', completedAt: new Date() });
