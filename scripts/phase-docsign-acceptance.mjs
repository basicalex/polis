  ...
  const visit = (candidate) => {
    ...
    for (const [key, nested] of Object.entries(candidate)) {
      assert.ok(
        ...
        `public projection leaked restricted key ${key}`,
      );
      visit(nested);
    }
  };
...
check(
  'public mandate-holder projection returns 200',
  initialProjection.status === 200,
  `status=${initialProjection.status}`,
);
check(
  'charterStatus is pending',
  initialProjection.body?.charterStatus === 'pending',
  `status=${initialProjection.body?.charterStatus}`,
);
...
check(
  'charterSignedDocumentHash is null',
  initialProjection.body?.charterSignedDocumentHash === null,
);

step(2, 'official is denied publication before signing');
const official = await citizenLogin(OFFICIAL_EMAIL);
check(
  'official identity is verified_official',
  official.citizen?.identityLevel === 'verified_official',
);
...
check(
  'unsigned publication returns 403',
  unsignedPublish.status === 403,
  `status=${unsignedPublish.status}`,
);
...
check(
  ...
  `status=${withoutKey.status}`,
);
check(
  'missing key error is idempotency_key_required',
  withoutKey.body?.error === 'idempotency_key_required',
);
...
check(
  'signing request has an id',
  typeof initiated.body?.id === 'string' && initiated.body.id.length > 0,
);
...
check(
  'repeated initiation returns 201',
  repeatedInitiation.status === 201,
  `status=${repeatedInitiation.status}`,
);
check(
  'repeated initiation returns the same request id',
  repeatedInitiation.body?.id === signingRequestId,
);
check(
  'repeated initiation returns the same unsigned artifact id',
  repeatedInitiation.body?.unsignedArtifactId === initiated.body?.unsignedArtifactId,
);
...
check(
  'different citizen initiation returns 403',
  otherInitiation.status === 403,
  `status=${otherInitiation.status}`,
);
check(
  'different citizen is rejected as non-owner',
  otherInitiation.body?.error === 'mandate_holder_owner_required',
  `error=${otherInitiation.body?.error}`,
);
...
check(
  'different citizen stub-complete returns 403',
  otherCompletion.status === 403,
  `status=${otherCompletion.status}`,
);
check(
  'different citizen is not a signing recipient',
  otherCompletion.body?.error === 'signing_recipient_required',
  `error=${otherCompletion.body?.error}`,
);
...
check(
  'completed request has signedArtifactId',
  typeof completed.body?.signedArtifactId === 'string' &&
    completed.body.signedArtifactId.length > 0,
);
check(
  'completed request has proofManifestId',
  typeof completed.body?.proofManifestId === 'string' && completed.body.proofManifestId.length > 0,
);
...
check(
  'repeated stub-complete remains 200',
  repeatedCompletion.status === 200,
  `status=${repeatedCompletion.status}`,
);
check(
  'repeated stub-complete preserves signedArtifactId',
  repeatedCompletion.body?.signedArtifactId === signedArtifactId,
);
check(
  'repeated stub-complete preserves proofManifestId',
  repeatedCompletion.body?.proofManifestId === proofManifestId,
);
...
check(
  'reconcile preserves signedArtifactId',
  reconciled.body?.signedArtifactId === signedArtifactId,
);
...
check(
  'stored signing request returns 200',
  storedRequest.status === 200,
  `status=${storedRequest.status}`,
);
check(
  'stored request preserves the single artifact id',
  storedRequest.body?.signedArtifactId === signedArtifactId,
);
check(
  'stored request preserves the single proof id',
  storedRequest.body?.proofManifestId === proofManifestId,
);
...
check(
  'public proof status returns 200',
  proofStatus.status === 200,
  `status=${proofStatus.status}`,
);
check(
  'public proof registryStatus is active',
  proofStatus.body?.registryStatus === 'active',
  `registryStatus=${proofStatus.body?.registryStatus}`,
);
...
check(
  'public projection has signed document hash',
  typeof signedDocumentHash === 'string' && signedDocumentHash.length > 0,
);
const verifiedHash = await postPublic('/api/v1/verify/hash', { hash: signedDocumentHash });
check(
  'signed document hash verification returns 200',
  verifiedHash.status === 200,
  `status=${verifiedHash.status}`,
);
check(
  'signed document hash verifies as valid',
  verifiedHash.body?.status === 'valid',
  `status=${verifiedHash.body?.status}`,
);
check(
  'hash verification resolves the same proof',
  verifiedHash.body?.manifest?.id === proofManifestId,
);

step(9, 'accepted public charter projection is complete and safe');
check(
  'charterStatus is accepted',
  acceptedProjection.body?.charterStatus === 'accepted',
  `status=${acceptedProjection.body?.charterStatus}`,
);
check('charterAccepted is true', acceptedProjection.body?.charterAccepted === true);
check(
  'charterSignedAt is present',
  typeof acceptedProjection.body?.charterSignedAt === 'string' &&
    acceptedProjection.body.charterSignedAt.length > 0,
);
check(
  'charterProofId matches the completed request',
  acceptedProjection.body?.charterProofId === proofManifestId,
);
check(
  'charterSignedDocumentHash remains present',
  acceptedProjection.body?.charterSignedDocumentHash === signedDocumentHash,
);
assertNoRestrictedCharterData(acceptedProjection.body);
console.log(
  '  ok  public projection contains no recipient, envelope, storage, or document-byte fields',
);
...
check(
  'out-of-scope commitment returns 403',
  outOfScope.status === 403,
  `status=${outOfScope.status}`,
);
check(
  'out-of-scope denial uses charter_scope_not_covered',
  outOfScope.body?.error === 'charter_scope_not_covered',
  `error=${outOfScope.body?.error}`,
);
...
check(
  'review fixture submission returns 201',
...
check(
  'non-staff review decision returns 403',
  nonStaffDecision.status === 403,
  `status=${nonStaffDecision.status}`,
);
check(
  'non-staff denial uses staff_required',
  nonStaffDecision.body?.error === 'staff_required',
  `error=${nonStaffDecision.body?.error}`,
);
...
check(
  'staff decision without body reviewer identity returns 201',
  staffDecision.status === 201,
  `status=${staffDecision.status}`,
);
check(
  'stored reviewer id comes from staff session',
  staffDecision.body?.review?.reviewerId === staff.citizen?.id,
  `reviewerId=${staffDecision.body?.review?.reviewerId}`,
);
